...
### Iteración 16 - Smoke Test UX y Correcciones de Interfaz del Widget
* **[Smoke Test Completado]**: Se realizó prueba humana E2E completa. Todos los pasos funcionales pasaron (apertura, Epic View, RAG con Gemini 2.5 Flash, memoria conversacional, cita de fuentes con nombre real de documento).
* **[Fix Fuentes RAG]**: Se refactorizó la query SQL en `rag.service.ts` agregando un `JOIN` con la tabla `documents` para extraer el `titulo` real del archivo en lugar del fallback genérico `'Este o múltiples documentos en Base de Datos'`.
* **[Cache Busting]**: Descubierta la causa raíz de por qué los fixes del widget no se aplicaban: `widget-demo.view.js` cargaba `main.js` con URL fija sin invalidación de caché. Se corrigió agregando `?t=Date.now()` al src del script y CSS.
* **[Bugs Pendientes — Widget UI]**: Se detectaron 4 problemas de UX que requieren validación adicional: (1) borde superior del panel con `border-radius` no clipea correctamente al header, (2) sin scroll automático al recibir respuesta, (3) pérdida de foco del input tras enviar, (4) respuesta del LLM se muestra como texto plano con asteriscos MD sin procesar. Se reescribió `app.ts` múltiples veces con correcciones (DomSanitizer, effect+rAF, ViewChild, CSS clip-path) pero requieren validación post-cache-bust en siguiente sesión.